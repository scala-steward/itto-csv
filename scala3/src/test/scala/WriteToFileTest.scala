...
class WriteToFileTest extends munit.FunSuite:
  ...
  test("writeEmptyListToFile") {
    import java.util.UUID

    final case class Bar(id: UUID, name: String, date: LocalDateTime)

    val filePath = Files.createTempFile("test", ".csv").toString
  ...
  test("writeListToFile") {
...

    {
      ...
      file.close()
      assert(
        lines == "id\tname\tdate\n1cc3ccbb-c749-3078-e050-1aacbe064651\tbob\t2018-11-20T09:10:25\n3cc3ccbb-c749-3078-e050-1aacbe064653\talice\t2018-11-20T10:12:24\n4cc3ccbb-c749-3078-e050-1aacbe064654\tjim\t2018-11-20T11:18:17\n5cc3ccbb-c749-3078-e050-1aacbe064655\ttom\t2018-11-20T11:36:04\n"
      )
    }
